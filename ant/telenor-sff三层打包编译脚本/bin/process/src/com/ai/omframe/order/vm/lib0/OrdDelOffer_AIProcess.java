...
public class OrdDelOffer_AIProcess implements ProcessInstance{
  private static transient Log logger = LogFactory.getLog(OrdDelOffer_AIProcess.class);
  protected IOVOrderOffer executeInner(ISoOfferData aSoOfferData,ISoUserData aSoUserData,OrderValueChain aOrderValueChain,IOVOrderCustomer aOVOrderCustomer,String REGION_ID) throws Exception{
	...
	long aOfferInstId = 0;
	long userId = 0;
    userId=aSoUserData.getUserId(); aOfferInstId=aSoOfferData.getInsOfferId(); int delState=com.ai.omframe.order.valuebean.OrderConst.DATA_DELETE_STATE; java.sql.Timestamp sysdate = com.ai.common.util.TimeUtil.getPrimaryDataSourceSysDate(); if(aSoOfferData.getExpireDate()!=null && aSoOfferData.getExpireDate().after(com.ai.common.util.TimeUtil.addOrMinusSecond(sysdate.getTime(), com.ai.omframe.util.SoUtil.getExpireDelayTime()))){ delState=com.ai.omframe.order.valuebean.OrderConst.DATA_MODIFY_STATE; } aOrderValueChain.setSTATE_PRICE_ATTR_OFFER(delState); aOrderValueChain.setSTATE_OFFER(delState);;
    ...
    while(aSoOfferData.getSoOfferPriceAttrData()!=null&&offPrcAttrIndex<aSoOfferData.getSoOfferPriceAttrData().length){
      ...
      aOfferPrcAttrValue = ((com.ai.omframe.instance.service.interfaces.IInstPriceAttr)com.ai.comframe.vm.common.ServiceUtil.getService("com.ai.omframe.instance.service.InstPriceAttr",com.ai.omframe.instance.service.interfaces.IInstPriceAttr.class)).getInstPriceAttrByRelatInstIdAndAttrId(aOfferInstId,priceAttrId,userId,price_inst_type,REGION_ID);
