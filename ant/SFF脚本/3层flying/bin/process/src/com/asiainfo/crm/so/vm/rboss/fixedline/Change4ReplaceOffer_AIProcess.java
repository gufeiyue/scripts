...
public class Change4ReplaceOffer_AIProcess implements ProcessInstance{
  private static transient Log logger = LogFactory.getLog(Change4ReplaceOffer_AIProcess.class);
  protected IOVOrderCustomer executeInner(long soUserDataKey,String REGION_ID,IOVOrderCustomer aOVOrderCustomer) throws Exception{
    ...
    {//Call the sub-processes换套餐
      ...
      tmpMap8.clear();
      tmpMap8.put("aSoOfferDatas",aOldSoOfferDatas);
      tmpMap8.put("aOper","DELETE");
      tmpMap8.put("REGION_ID",REGION_ID);
      tmpMap8.put("aOVOrderCustomer",aOVOrderCustomer);

      com.ai.comframe.vm.processflow.ProcessEngineFactory.getProcessEngine().executeProcess("com.asiainfo.crm.so.vm.rboss.fixedline.OrdOperOffer",tmpMap8);
