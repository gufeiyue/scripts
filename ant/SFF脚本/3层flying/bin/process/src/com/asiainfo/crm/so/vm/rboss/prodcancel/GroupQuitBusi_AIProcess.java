...
public class GroupQuitBusi_AIProcess implements ProcessInstance{
  private static transient Log logger = LogFactory.getLog(GroupQuitBusi_AIProcess.class);
  protected IOVOrderOffer[] executeInner(long customerOrderId,long soOfferDataKey,String REGION_ID,IOVOrderOffer[] ovOrderOffers) throws Exception{
  ...
  public IOVOrderOffer[] execute(long customerOrderId,long soOfferDataKey,String REGION_ID,IOVOrderOffer[] ovOrderOffers) throws Exception{
    Map $inParameter = new HashMap();
    Map $returnParameter = new HashMap();
    $inParameter.put("customerOrderId",new Long(customerOrderId));
    $inParameter.put("soOfferDataKey",new Long(soOfferDataKey));
    $inParameter.put("REGION_ID",REGION_ID);
    $inParameter.put("ovOrderOffers",ovOrderOffers);
try{
    IOVOrderOffer[] realReturn = executeInner( customerOrderId, soOfferDataKey, REGION_ID, ovOrderOffers);
...
if(e instanceof RuntimeException){
 ...
 throw new RuntimeException(e);
  ...
  public Map execute(Map $vmParameters) throws Exception{
    ...
    IOVOrderOffer[] ovOrderOffers = ($vmParameters.get("ovOrderOffers") == null)?null:(IOVOrderOffer[])VMDataType.transfer($vmParameters.get("ovOrderOffers"),IOVOrderOffer[].class);
IOVOrderOffer[] realReturn = execute(customerOrderId,soOfferDataKey,REGION_ID,ovOrderOffers);
