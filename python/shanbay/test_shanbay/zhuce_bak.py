...
def download_captcha_1(ck,captcha_0):
...
def judge(ck,sub_name, sub_password, sub_email,ref_url):
    lists = [1, 2]
    captcha_0 = download_captcha_0(ck)
    captcha_1 = download_captcha_1(ck,captcha_0)
    ...
    if s['status_code'] == 0:
        lists[0] = captcha_0
        lists[1] = captcha_1
        r = zhuce(ck,sub_name, sub_password, sub_email, captcha_0, captcha_1, ref_url)
    else:
        judge(ck,sub_name, sub_password, sub_email,ref_url)



def zhuce(ck,uname, pwd, email, captcha_0, captcha_1, ref_url):
    r = requests.get('https://www.shanbay.com/accounts/register/?ref_code=e93082dfc0',cookies=ck)
    #print r
    cookie = dict(csrftoken=r.cookies['csrftoken'])
...
def db_update_available(sub_name):
    update_sql = "update `test_account` set `is_available`='N' where `sub_name` = '%s';"
...
def db_update_parent(parent_name,parent_password,sub_name):
    update_sql = "update `test_account` set `parent_name`='%s',`parent_password`='%s' where `sub_name`='%s';"
...
def test():
    ...
    parent_name = "honeybunnypepper"
    parent_password = "lplpllp"
    ...
    #ref_url = "https://www.shanbay.com/accounts/register/?ref_code=477d6ba31f" 


    select_sql = "select * from test_account where `is_available`='Y'  limit %s;"
    sql = select_sql % number
    info = db_query(sql)

    acct_sql = "select * from shanbay_account where `is_available`='N' order by rand() limit 140;"
    acctinfo = db_query(acct_sql)
    
    #print info
    i = number
    while i >= 1:
        print colored('=========这是第 %s 个帐号===========', 'red') % i
        acct_name = acctinfo[number-i][3]
        acct_password = acctinfo[number-i][5]
        print "acct_name name:",acct_name
        ...
        ck = login(acct_name, acct_password)
        judge(ck,sub_name, sub_password, sub_email,ref_url)
