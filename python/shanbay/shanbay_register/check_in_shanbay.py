...
def read(nid, ck):
    ...
    if 0 == s['status_code']:
        ...
        data = {'used_time': 479, 'operation': 'finish'}
...
def db_update_days(sub_name,days,shells):
    update_sql = "update `shanbay_account` set `times`='%s',`punch`='U',`shells`='%s' where `sub_name` = '%s';"
...
def test_parent_sub():
    ...
    select_sql = "select * from shanbay_account where `parent_name`='%s';"
...
def test_all():
    sql = "select * from shanbay_account where `times`< 8 and `is_available`='N' and `punch`='E';"
    info = db_query(sql)
    number = num(sql)
    i = number
    while i >= 1:
